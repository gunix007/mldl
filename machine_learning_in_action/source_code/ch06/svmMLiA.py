...
# helper functions for the SMO algrithm, start
...
def clipAlpha(aj, H, L):
    ...
    if (L > aj):
        aj = L
    return aj
# helper functions for the SMO algrithm, end
...
# supporting functions for full Platt SMO, start
'''
class optStruct:
...
# complete platt smo outer iteration
...
# kernel function for full Platt SMO
class optStruct:
...
# inner iteration for kernel function
def innerL(i, oS):
    Ei = calcEk(oS, i)
    if ((oS.labelMat[i] * Ei < -oS.tol) and (oS.alphas[i] < oS.C)) \
            or ((oS.labelMat[i] * Ei > oS.tol) and (oS.alphas[i] > 0)):
